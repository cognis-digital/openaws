...
**openaws** plugs into your stack through [`cognis-connect`](https://github.com/cognis-digital/cognis-connect),
...
Once `openaws` emits JSON findings, pipe them straight to a destination — `--dry-run`
previews the exact request without sending:

```bash
openaws ... --format json | cognis-connect emit --to stix   # STIX 2.1 bundle
openaws ... --format json | cognis-connect emit --to slack --url $URL --token $TOK   # Slack
openaws ... --format json | cognis-connect emit --to webhook --url $URL --token $TOK   # generic webhook
```
...
works whatever `openaws` outputs:

```python
from cognis_connect import normalize, stix
findings = [normalize(rec, source="openaws") for rec in records]   # records = your JSON output
