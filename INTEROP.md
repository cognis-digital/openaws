...
How **openaws** fits the wider Cognis suite — a composable set of defensive, analytical
tools that all run on your own hardware and speak plain **JSON** (or an OpenAI-compatible
**`/v1`**). `openaws` lives in the **General OSINT / security tool** cluster.
...
| from | relation | to |
|---|---|---|
| `openaws` | JSON findings compose with | [`awesome-cognis`](https://github.com/cognis-digital/awesome-cognis) |
| `openaws` | AI add-ins are served `/v1` by | [`edgemesh`](https://github.com/cognis-digital/edgemesh) (your fleet) |
| `openaws` | findings can be narrated / reasoned over by | [`humind`](https://github.com/cognis-digital/humind) -> [`agentlex`](https://github.com/cognis-digital/agentlex) |
| `openaws` | export to intel formats via | [`stixgen`](https://github.com/cognis-digital/stixgen) (STIX) / [`attackmap`](https://github.com/cognis-digital/attackmap) (ATT&CK) |

## Composition patterns

Everything reads/writes JSON, so tools chain with ordinary pipes; nothing leaves the box.

**1 — chain within the general osint / security tool cluster.** `openaws` output feeds the next tool:
```bash
openaws ... --format json > out.json          # this tool's findings (see `openaws --help`)
...
openaws ...                                         # vision / reasoning add-ins light up
```

**3 — export findings to your SOC's formats.**
```bash
openaws ... --format json | stixgen from-json > bundle.stix.json   # STIX 2.1
openaws ... --format json | attackmap map > attack.json            # ATT&CK techniques
```

**4 — narrate through cognition + agents.** `humind` extracts salience from `openaws`'s
output; `agentlex` holds it as KB facts and fires Horn rules to escalate. `openaws` slots
